  ...
  try {
    ...
    if (adminExists.rowCount === 0) {
      await sql`
        ...
        VALUES ('Admin', ${demoEmail}, 'admin', 'https://api.dicebear.com/7.x/avataaars/svg?seed=Admin', ${hashedPassword})
      `;
    } else {
        await sql`UPDATE users SET password = ${hashedPassword} WHERE email = ${demoEmail} AND password IS NULL`;
    }
